...
app.use((err, req, res, next) => {
    if (err instanceof SyntaxError && err.status === 400 && 'body' in err) {
        return res.status(400).json({ success: false, error: 'تنسيق JSON غير صالح في الطلب' });
    }
    next();
});
...
app.get('/api/system/stats', async (req, res) => {
    try {
        const settingsCount = (await dbGet('SELECT COUNT(*) as count FROM settings')).count;
        const newsCount = (await dbGet('SELECT COUNT(*) as count FROM news')).count;
        const initiativesCount = (await dbGet('SELECT COUNT(*) as count FROM initiatives')).count;
        const articlesCount = (await dbGet('SELECT COUNT(*) as count FROM articles')).count;
        const galleryCount = (await dbGet('SELECT COUNT(*) as count FROM gallery')).count;
        const achievementsCount = (await dbGet('SELECT COUNT(*) as count FROM achievements')).count;
        const messagesCount = (await dbGet('SELECT COUNT(*) as count FROM messages')).count;
        const newMessagesCount = (await dbGet("SELECT COUNT(*) as count FROM messages WHERE status = 'جديد'")).count;
...
app.post('/api/settings', async (req, res) => {
    try {
        ...
        if (!key) return res.status(400).json({ success: false, error: 'مفتاح الإعداد مطلوب' });
...
app.post('/api/upload', (req, res) => {
    try {
        ...
        if (!image) return res.status(400).json({ success: false, error: 'لم يتم إرسال ملف الصورة' });

        const matches = image.match(/^data:image\/([a-zA-Z0-9]+);base64,(.+)$/);
        if (!matches) {
            return res.status(400).json({ success: false, error: 'تنسيق صورة Base64 غير صحيح' });
        }
...
app.post('/api/stats', async (req, res) => {
    try {
        ...
        if (!key_name || !label) {
            return res.status(400).json({ success: false, error: 'اسم المفتاح والعنوان مطلوبان' });
        }
        const result = await dbRun(
            'INSERT INTO stats (key_name, label, value_number, icon, sort_order) VALUES (?, ?, ?, ?, ?)',
            [key_name, label, parseInt(value_number) || 0, icon || 'fa-chart-bar', parseInt(sort_order) || 0]
        );
...
app.get('/api/news/:id', async (req, res) => {
    try {
        ...
        if (!row) return res.status(404).json({ success: false, error: 'الخبر غير موجود' });
...
app.post('/api/news', async (req, res) => {
    try {
        ...
        if (!title || !category || !snippet || !content) {
            return res.status(400).json({ success: false, error: 'جميع حقول الخبر الأساسية مطلوبة' });
        }
        const result = await dbRun(
            'INSERT INTO news (title, category, snippet, content, date, image_url) VALUES (?, ?, ?, ?, ?, ?)',
            [title, category, snippet, content, date || new Date().toISOString().split('T')[0], image_url || 'https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?w=800']
        );
        res.json({ success: true, id: result.id });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
...
app.get('/api/initiatives/:id', async (req, res) => {
    try {
        const row = await dbGet('SELECT * FROM initiatives WHERE id = ?', [req.params.id]);
        if (!row) return res.status(404).json({ success: false, error: 'المبادرة غير موجودة' });
        res.json({ success: true, data: row });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
...
app.post('/api/initiatives', async (req, res) => {
    try {
        ...
        if (!title || !category || !description) {
            return res.status(400).json({ success: false, error: 'العنوان والتصنيف والوصف حقول مطلوبة' });
        }
        const result = await dbRun(
            'INSERT INTO initiatives (title, category, description, status, date, image_url) VALUES (?, ?, ?, ?, ?, ?)',
            [title, category, description, status || 'قيد التنفيذ', date || new Date().toISOString().split('T')[0], image_url || 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=800']
        );
        res.json({ success: true, id: result.id });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
...
app.get('/api/articles/:id', async (req, res) => {
    try {
        const row = await dbGet('SELECT * FROM articles WHERE id = ?', [req.params.id]);
        if (!row) return res.status(404).json({ success: false, error: 'المقال غير موجود' });
        res.json({ success: true, data: row });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
...
app.post('/api/articles', async (req, res) => {
    try {
        const { title, author, category, snippet, content, date, read_time } = req.body;
        if (!title || !category || !snippet || !content) {
            return res.status(400).json({ success: false, error: 'جميع حقول المقال الأساسية مطلوبة' });
        }
        const result = await dbRun(
            'INSERT INTO articles (title, author, category, snippet, content, date, read_time) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [title, author || 'اسم الشخص', category, snippet, content, date || new Date().toISOString().split('T')[0], read_time || '5 دقائق']
        );
        res.json({ success: true, id: result.id });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
...
app.post('/api/gallery', async (req, res) => {
    try {
        ...
        if (!title || !category || !image_url) {
            return res.status(400).json({ success: false, error: 'العنوان والتصنيف ورابط الصورة حقول مطلوبة' });
        }
        const result = await dbRun(
            'INSERT INTO gallery (title, category, image_url, date) VALUES (?, ?, ?, ?)',
            [title, category, image_url, date || new Date().toISOString().split('T')[0]]
        );
        res.json({ success: true, id: result.id });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
...
app.post('/api/achievements', async (req, res) => {
    try {
        ...
        if (!title || !category || !description) {
            return res.status(400).json({ success: false, error: 'جميع حقول الإنجاز مطلوبة' });
        }
        const result = await dbRun(
            'INSERT INTO achievements (title, category, description, year) VALUES (?, ?, ?, ?)',
            [title, category, description, year || new Date().getFullYear().toString()]
        );
        res.json({ success: true, id: result.id });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
...
app.post('/api/messages', async (req, res) => {
    try {
        ...
        if (!emailRegex.test(email)) {
            return res.status(400).json({ success: false, error: 'تنسيق البريد الإلكتروني غير صحيح' });
        }
        const result = await dbRun(
            'INSERT INTO messages (name, email, phone, subject, details, status) VALUES (?, ?, ?, ?, ?, ?)',
            [name.trim(), email.trim(), phone ? phone.trim() : '', subject.trim(), details.trim(), 'جديد']
        );
        res.json({ success: true, id: result.id, message: 'تم إرسال الرسالة بنجاح وسوف يتم التواصل معكم قريباً.' });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
});
...
app.get('/api/search', async (req, res) => {
    try {
        ...
        if (!q) {
            return res.json({ success: true, data: { news: [], initiatives: [], articles: [], achievements: [] } });
        }
        const searchTerm = `%${q}%`;
        const news = await dbAll('SELECT id, title, category, snippet, date FROM news WHERE title LIKE ? OR snippet LIKE ? OR content LIKE ? LIMIT 10', [searchTerm, searchTerm, searchTerm]);
        const initiatives = await dbAll('SELECT id, title, category, description, status FROM initiatives WHERE title LIKE ? OR description LIKE ? LIMIT 10', [searchTerm, searchTerm]);
        const articles = await dbAll('SELECT id, title, category, snippet, author, date FROM articles WHERE title LIKE ? OR snippet LIKE ? OR content LIKE ? LIMIT 10', [searchTerm, searchTerm, searchTerm]);
        const achievements = await dbAll('SELECT id, title, category, description, year FROM achievements WHERE title LIKE ? OR description LIKE ? LIMIT 10', [searchTerm, searchTerm]);
...
app.get('/api/db/export', async (req, res) => {
    try {
        ...
        res.json({
            ...
            data: { settings, stats, news, initiatives, articles, gallery, achievements, messages }
        });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
...
app.post('/api/sql/query', async (req, res) => {
    try {
        ...
        if (!query || typeof query !== 'string') {
            return res.status(400).json({ success: false, error: 'استعلام SQL فارغ أو غير صحيح' });
        }
        ...
        if (trimmed.startsWith('SELECT') || trimmed.startsWith('PRAGMA') || trimmed.startsWith('EXPLAIN')) {
            ...
            res.json({ success: true, type: 'select', durationMs, count: rows.length, data: rows });
        } else {
            const result = await dbRun(query);
            const durationMs = Date.now() - startQueryTime;
            res.json({ success: true, type: 'mutation', durationMs, changes: result.changes, lastID: result.id });
        }
    } catch (err) {
...
async function startServer() {
    ...
    if (!fs.existsSync(sslKeyPath) || !fs.existsSync(sslCertPath)) {
        try {
            ...
            fs.writeFileSync(sslKeyPath, pems.private);
            fs.writeFileSync(sslCertPath, pems.cert);
            console.log('🔒 تم إنشاء شهادة SSL ذاتية التوقيع بنجاح في مجلد ssl/');
        } catch (err) {
            console.error('⚠️ تعذر إنشاء شهادة SSL تلقائياً:', err.message);
        }
    }
    ...
    // 1. Start HTTPS Server (Primary on Port 3000)
    if (fs.existsSync(sslKeyPath) && fs.existsSync(sslCertPath)) {
        try {
            const sslOptions = {
            ...
            https.createServer(sslOptions, app).listen(HTTPS_PORT, () => {
                console.log(`🔒 (HTTPS) المنصة تعمل بشهادة مشفّرة على: https://localhost:${HTTPS_PORT}`);
                console.log(`🌐 صفحة الزائر العامة: https://localhost:${HTTPS_PORT}/index.html`);
                console.log(`🔑 لوحة التحكم: https://localhost:${HTTPS_PORT}/admin.html`);
            });
        } catch (err) {
            console.error('❌ خطأ في تشغيل خادم HTTPS:', err.message);
        }
    }

    // 2. Start HTTP Server (Secondary on Port 3001)
    http.createServer(app).listen(HTTP_PORT, () => {
        console.log(`🚀 (HTTP)  المنصة تعمل أيضاً على: http://localhost:${HTTP_PORT}`);
        console.log(`🌐 صفحة الزائر العامة: http://localhost:${HTTP_PORT}/index.html`);
        console.log(`🔑 لوحة التحكم: http://localhost:${HTTP_PORT}/admin.html`);
    });
}
